...
export class CargoArchitectPlanner extends Component {
    ...
    toggleShowCoordinates(ev) {
        this.state.showCoordinates = !!ev.target.checked;
    }
    getProductKey(p) {
        const name = String(p.name || "").trim();
        return (name.split(/\s+/)[0] || name || `Item-${p.id}`);
    }
    colorForKey(key) {
        const palette = ["#60a5fa", "#34d399", "#fbbf24", "#f87171", "#a78bfa", "#22d3ee", "#fb7185", "#fb923c", "#4ade80", "#818cf8", "#f472b6", "#2dd4bf"];
        let hash = 0;
    ...
    applyPlacementDisplayMetadata() {
        this.state.placements.forEach((p, index) => {
            const key = this.getProductKey(p);
            const color = this.colorForKey(key);
            p.item_number = index + 1;
            p.product_key = key;
            p.item_color = color;
            p.row_style = `background-color: ${this.hexToRgba(color, 0.18)}; border-left: 6px solid ${color};`;
        });
    }
    setActiveView(mode) {
    ...
    async reload() {
        ...
        this.state.placements = await this.orm.searchRead("cargo.architect.placement", [["plan_id", "=", this.planId]], [
            "name", "x_in", "y_in", "z_in", "length_in", "width_in", "height_in", "weight_lbs", "locked", "rotated"
        ], { order: "sequence asc, id asc" });
        this.applyPlacementDisplayMetadata();
    ...
    drawView(mode) {
        ...
        if (mode === "left" || mode === "right") {
            ...
            placements.forEach((p, i) => {
                ...
                ctx.fillStyle = p.item_color || this.colorForKey(this.getProductKey(p));
                ...
                if (w > 18 && h > 12) {
                    ...
                    const label = String(p.item_number || (i + 1));
                    const textW = ctx.measureText(label).width;
                    ctx.fillText(label, x + Math.max(3, (w - textW) / 2), y + Math.max(13, h / 2));
                    if (p.rotated && h > 24) {
                        ctx.font = "bold 11px sans-serif";
                        ctx.fillText("R", x + w - 13, y + 13);
                    }
                }
            });
        } else {
            ...
            this.state.placements.forEach((p, i) => {
                ...
                if (w > 18 && h > 12) {
                    ...
                    const textW = ctx.measureText(label).width;
                    ctx.fillText(label, x + Math.max(3, (w - textW) / 2), y + Math.max(13, h / 2));
                    if (p.rotated && w > 30 && h > 22) {
                        ctx.font = "bold 10px sans-serif";
                        ctx.fillText("R", x + w - 12, y + 12);
                    }
                }
            });
        }
