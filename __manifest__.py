...
# v2.0.47 Odoo Store Submission Package: adds polished static description page, screenshot image assets, final OPL-1/store metadata, and submission-readiness documentation.

# v2.0.47 Rotation Fallback: optimizer now tries the base footprint first and only uses length/width rotation when the unrotated orientation cannot be physically placed.
# v2.0.48 Minimal Rotation Enforcement: dense compaction and transport-block pairing preserve base L/W orientation and use swapped L/W only as a true fit fallback.
